...
The skill-level eval suite measures whether a real host agent (Claude Code or Codex) can use the
three backtrader skills to produce reviewed, runnable strategies, and whether the review gates hold
against adversarial requests. Ten prompts live in `prompts/`; a deterministic scorer records the
mechanical outcomes; a human scores behavior from the session transcript.
