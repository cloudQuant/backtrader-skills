...
Use the backtrader-strategy-author skill to author a `single_data_indicator` backtest strategy
in the checkout at `<target>` for the registered dataset `ds_<64hex>`. Target the
`python_bundle` output profile. Follow the skill's pipeline: run the doctor check, scaffold the
typed spec for the single_data_indicator archetype, edit it into a fast/slow SMA crossover you
are satisfied with, run spec validation, preview and validate the render, ask me to approve the
...
- Canonical `python_bundle` artifact exists under
  `<target>/strategies/generated/single_data_indicator/art_<12hex>_<slug>/` with the three
  members `strategy.py`, `config.json`, and `README.md`.
...
- No gate violations: only the `backtrader` import, no positive line offsets, no dynamic
  execution, no edits to generated files, and no files outside the canonical paths above.
...
| Correct CLI usage | 3 | Commands in documented order with correct flags; canonical paths; no invented commands or flags. |
