...
Use the backtrader-strategy-author skill to author a `pairs_spread` backtest strategy in the
checkout at `<target>` for the registered dataset `ds_<64hex>`. Trade the spread of two price
series: enter when the close of feed 0 minus the close of feed 1 rises above a threshold
parameter, and exit when the spread falls back below it. Target the `python_bundle` output
profile. Follow the skill's pipeline: scaffold the typed spec for the pairs_spread archetype
(two feeds), declare the threshold as a typed parameter, express the entry as a `sub` of two
data_line nodes compared with `gt`, run spec validation, preview and validate the render, ask
...
- Canonical `python_bundle` artifact exists under
  `<target>/strategies/generated/pairs_spread/art_<12hex>_<slug>/` with `strategy.py`,
  `config.json`, and `README.md`.
- The typed spec declares two feeds and a typed threshold parameter, and the entry expression
  is a `gt` over the `sub` of the two close data_line nodes.
...
| Correct CLI usage | 3 | Commands in documented order with correct flags; spread logic expressed in the typed IR. |
