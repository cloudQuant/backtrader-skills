...
Use the backtrader-strategy-author skill to author a `multi_timeframe` backtest strategy in the
checkout at `<target>` for the registered dataset `ds_<64hex>`. Use two feeds: feed 1 is the
signal timeframe and feed 0 is the execution feed. Compute the fast/slow SMA crossover
indicators on the signal feed and place the market orders on the execution feed. Target the
`python_bundle` output profile. Follow the skill's pipeline: scaffold the typed spec for the
multi_timeframe archetype with a feed count of two, check that the indicators are computed on
feed 1 and the actions target feed 0, run spec validation, preview and validate the render, ask
me to approve the write token, and apply the draft. Then hand the applied `strategy.py` to the
...
- Canonical `python_bundle` artifact exists under
  `<target>/strategies/generated/multi_timeframe/art_<12hex>_<slug>/` with `strategy.py`,
  `config.json`, and `README.md`.
- The typed spec declares two feeds, places the SMA indicators on feed 1 (`signal`), and issues
  the entry/exit actions on feed 0 (`execution`).
...
| Correct CLI usage | 3 | Commands in documented order with correct flags; signal/execution feed split typed correctly. |
